...
const replaySpeedReadout = document.getElementById('replay-speed-readout');
...
/* Log-scale mapping: slider 0\u2013100 \u2194 speed 0.25\u00d7\u201316\u00d7.
   Using 2^(position-based exponent) so each equal slider step
   is an equal *perceptual* speed change. Position 50 = 1\u00d7. */
const SPEED_MIN_EXP = -2;
const SPEED_MAX_EXP = 4;

function sliderToSpeed(sliderValue) {
  const pct = sliderValue / 100;
  const exp = SPEED_MIN_EXP + pct * (SPEED_MAX_EXP - SPEED_MIN_EXP);
  return Math.pow(2, exp);
}

function speedToSlider(speed) {
  const exp = Math.log2(speed);
  const pct = (exp - SPEED_MIN_EXP) / (SPEED_MAX_EXP - SPEED_MIN_EXP);
  return Math.max(0, Math.min(100, pct * 100));
}

function formatSpeed(speed) {
  if (speed >= 10) return `${speed.toFixed(0)}\u00d7`;
  if (speed >= 1) return `${speed.toFixed(1).replace(/\.0$/, '')}\u00d7`;
  return `${speed.toFixed(2).replace(/0$/, '')}\u00d7`;
}

function currentSpeed() {
  return sliderToSpeed(parseFloat(replaySpeed.value));
}
...
/* 'input' fires continuously while dragging; 'change' only on release.
   'input' is the standard choice for live slider feedback. */
replaySpeed.addEventListener('input', () => {
  replaySpeedReadout.textContent = formatSpeed(currentSpeed());
  if (replayState.playing) {
    const current = replayState.startedFrom +
      (performance.now() - replayState.startedAt) * currentSpeed();
    replayState.startedFrom = current;
    replayState.startedAt = performance.now();
  }
});

// For red-flagged students, default playback faster since the
// interesting signal is the pattern, not the text.
if (submission.flag === 'red') {
  replaySpeed.value = speedToSlider(4).toString();
}

// Initialize readout on load
replaySpeedReadout.textContent = formatSpeed(currentSpeed());
